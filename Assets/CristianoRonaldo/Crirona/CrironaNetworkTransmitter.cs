...
public class CrironaNetworkTransmitter : NetworkBehaviour
{

    private static readonly string LOG_PREFIX = "[" + typeof(CrironaNetworkTransmitter).Name + "]: ";
    private const int RELIABLE_SEQUENCED_CHANNEL = CrironaNet.ChannelReliableSequenced;
    ...
    {
        Debug.Assert(serverTransmissionIds.Contains(transmissionId) == false);
    ...
    private void RpcReceiveBytes(int transmissionId, byte[] recBuffer)
    {
        ...
        if (clientTransmissionData.ContainsKey(transmissionId) == false)
            return;
